...
int main(int argc,char * argv[]){
    ...
    if(argc == 4){
        ...
        if(strcmp(argv[1],"diff") == 0){
            diffsnapshots(atoi(argv[2]),atoi(argv[3]));
        }
        else{
            printHelp();
        }
...
int diffsnapshots(int ver1,int ver2){
    if(ver1 > MAX_VERSIONS || ver1 < 0 || ver2 > MAX_VERSIONS || ver2 < 0){
        printf(1,"Invlaid Version\n");
        return 0;
    }
    if(ver1 > numLogs() || ver2 > numLogs()){
        printf(1,"Invalid Version,Either Version Does Not exist or Log is corrupted.\n");
        return 0;
    }

    if(ver1 == ver2){
        printf(1,"Same Versions,No Difference.\n");
        return 0;       
    }
    ...
    buf[17] = ver1 + '0';
    buf[18] = '\0';
    char buf1[20] ="snapshots/working";
    buf1[17] = ver2 + '0';
    buf1[18] = '\0';
    char * arg[] = {
        "",
...
void printHelp(){
    ...
    printf(1,"f] snapnroll diff  <version_no> <version_no> [version_no = {0..%d}]\n",MAX_VERSIONS - 1);
}
...
void dir_diff(char *dir1, char *dir2) {
    ...
    while (read(fd2, &de2, sizeof(de2)) == sizeof(de2)) {
        ...
        snprintf(path2, sizeof(path2), "%s/%s", dir2, de2.name);
